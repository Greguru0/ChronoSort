...
Function Import-Source-Files {
    ...
    foreach ($file in $sourceFiles) {
		...
		if ($destinationHashDictionary.Count -ne 0) {
			...
			$sourceHash = Get-Hash -FilePath $($file.FullName)
		}
...
function Get-ExtendedProperty {
    ...
    $mediaCreatedProperty = $ShellFolder.GetDetailsOf($ShellFile, 208)  # "Media Created" property
    $dateTakenProperty = $ShellFolder.GetDetailsOf($ShellFile, 12)    # "Date Taken" property
    $dateModifiedProperty = $ShellFolder.GetDetailsOf($ShellFile, 3)    # "Date Modified" property
    
    if ($mediaCreatedProperty) {
        return $mediaCreatedProperty
    }
    elseif ($dateTakenProperty) {
        return $dateTakenProperty
    }
    elseif ($dateModifiedProperty) {
        return $dateModifiedProperty
    } else {
        return "Neither 'Media Created' nor 'Date Taken' nor 'Date Modified' properties are available."
		pause
		exit
    }
}
...
function Rename-File {
	...
	$mediacreated = Get-ExtendedProperty -FilePath $sourceFilePath
	$mediacreated = ($mediacreated -split ' ')[0]
	$mediacreated = $mediacreated -replace '\s+|[^i\p{IsBasicLatin}/]|[^\x20-\x7E/]', ''
    $dateTime = [DateTime]::MinValue
    [void][DateTime]::TryParse($mediacreated, [ref]$dateTime)
